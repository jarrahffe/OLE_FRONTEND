...
import usyd_logo from '../../assets/usyd.jpg';
...
const NavBar = (props: Props) => {
  ...
  function handleUndo() {
    ...
    if (multiActivities) {
      for (const setActivity of (
        multiActivities as Map<Activity, Function>
      ).values()) {
        setActivity(undefined)
      }
    }
    if (multiDelete) {
      for (const setOpacity of (
        multiDelete as Map<Activity, Function>
      ).values()) {
        setOpacity("")
      }
    }
  ...
  function formatSwappedString(swapped: Activity) {
    return moment(
      `${swapped.date}-${swapped.start_time}`,
      "YYYY-MM-DD-H"
    ).format("ddd ha")
  }
  ...
  function handleSwapRequest() {
    ...
    createSwapRequest(
      (swappedFrom as Activity).id,
      (swappedTo as Activity).id,
      token,
      setBookProgress,
      setErrorTooltipActive,
      setBookFeedbackIcon,
      setSwapMenuModal,
      setSwappedFrom,
      setSwappedTo
    ...
    return (
      <LoadingButton
        ...
        sx={{
          ...
          right: "30%",
          top: "110%",
          color: swappedFrom && swappedTo ? "white" : "",
        }}
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            <Button
              color="success"
              onClick={() =>
                multiActivities || multiDelete
                  ? setMultiSelectModal(true)
                  : null
              }
              variant={
                multiActivities || multiDelete ? "contained" : "outlined"
              }
              size="small"
      ...
      >
        <Typography
          sx={{ display: "flex", justifyContent: "center", fontSize: "1rem" }}
        >
          {SESSION}
        </Typography>
          ...
          SESSION === "Semester Break" ? null : (
            <>
              ...
              <IconButton
                ...
                onClick={() => handleSliderChange(selectedWeek + 1)}
              >
              ...
              <Button
                ...
                onClick={() => handleSliderChange(CURRENT_WEEK)}
              >
                {selectedWeek < 14 ? `Week ${selectedWeek}` : "Exam Period"}
              </Button>
            </>
          )
        ) : (
      ...
      {swapMenuModal ? (
        <div className="navbar-swap-menu">
          <div className="navbar-swap-menu-time-glow-left">
            <div
              className="navbar-swap-menu-time"
              style={{
            ...
            >
              <Typography
                sx={{
                  fontWeight: 1000,
                  color: swappedFrom ? "gold" : "#1976d2",
                }}
              >
                {swappedFrom ? formatSwappedString(swappedFrom) : "SELECT"}
              </Typography>
            </div>
          </div>
          ...
          <div
            className="navbar-swap-menu-time-glow-right"
            style={{ right: "0%" }}
          >
            <div
              className="navbar-swap-menu-time"
            ...
            >
              <Typography sx={{ fontWeight: 1000 }}>
                {swappedTo
                  ? formatSwappedString(swappedTo as Activity)
                  : "SELECT"}
              </Typography>
            </div>
          </div>
      ...
      {swapHubTransition((style, modal) =>
        modal ? (
          <animated.div
            style={style}
            className="absolute right-0 top-[150%] z-10 flex max-h-[1250%] w-1/5 flex-col items-center justify-evenly rounded-md bg-gray-100 outline outline-[0.5px] outline-gray-500"
